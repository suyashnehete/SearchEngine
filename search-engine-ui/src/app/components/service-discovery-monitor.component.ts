...
@Component({
    ...
    <div class="discovery-monitor">
      <div class="monitor-header">
        ...
        <div class="eureka-status" [class]="discoveryStatus?.eurekaAvailable ? 'status-up' : 'status-down'">
          <span class="status-indicator"></span>
          Eureka {{ discoveryStatus?.eurekaAvailable ? 'Available' : 'Unavailable' }}
        </div>
      ...
      <div class="stats-row" *ngIf="discoveryStatus">
        ...
        <div class="stat-card">
          <div class="stat-value">{{ discoveryStatus.registeredServices.length }}</div>
          <div class="stat-label">Registered Services</div>
        </div>
      </div>

      <div class="services-section" *ngIf="discoveryStatus?.registeredServices && discoveryStatus!.registeredServices.length > 0">
        <h4>Registered Services</h4>
        <div class="services-list">
          <div *ngFor="let service of discoveryStatus?.registeredServices" class="service-item">
            <div class="service-header">
              <h5>{{ service.name }}</h5>
              <span class="instance-count">{{ service.instances.length }} instance(s)</span>
            </div>
            <div class="instances-list">
              <div *ngFor="let instance of service.instances" 
                   class="instance-item"
                   [class]="'status-' + instance.status.toLowerCase()">
                <div class="instance-info">
                  <span class="instance-id">{{ instance.instanceId }}</span>
                  <span class="instance-address">{{ instance.ipAddr }}:{{ instance.port }}</span>
                </div>
      ...
      <div class="communication-test" *ngIf="communicationResults">
        ...
        <div class="communication-grid">
          <div *ngFor="let result of getCommunicationResults()" 
        ...
        <button (click)="testCommunication()" [disabled]="isTestingCommunication" class="test-btn">
  ...
  `,
    ...
    .refresh-btn:hover:not(:disabled), .test-btn:hover:not(:disabled) {
      background: #0056b3;
    }
    ...
    .services-section, .communication-test, .gateway-routes {
      background: white;
      padding: 20px;
      border-radius: 6px;
      margin-bottom: 20px;
    ...
    .services-section h4, .communication-test h4, .gateway-routes h4 {
      margin: 0 0 15px 0;
      color: #495057;
    }
    ...
    .communication-item.status-up {
      background: #d4edda;
      border-color: #c3e6cb;
      color: #155724;
    }

    .communication-item.status-down {
      background: #f8d7da;
      border-color: #f5c6cb;
      color: #721c24;
    }
